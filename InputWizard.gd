...
static func is_mouse_left_click(event: InputEvent) -> bool:
	return is_mouse_button(event) and event.button_index == MOUSE_BUTTON_LEFT && event.pressed


static func is_mouse_right_click(event: InputEvent) -> bool:
	return is_mouse_button(event) and event.button_index == MOUSE_BUTTON_RIGHT && event.pressed
...
static func get_keyboard_input_for_action(action: String) -> InputEvent:
	var inputs: Array[InputEvent] = InputWizard.get_keyboard_inputs_for_action(action)
